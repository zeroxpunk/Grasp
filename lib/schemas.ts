...
/**
 * Flat exercise schema for AI structured output generation.
 * No superRefine — Anthropic structured output needs a clean JSON schema.
 * Runtime validation of type-specific required fields happens in normalizeExercise().
 */
  ...
  lessonContents: z
    .array(
      z.object({
        lessonNumber: z
          .number()
          .describe("Lesson number this content belongs to (1 or 2)"),
        content: z
          .string()
          .describe(
            "Full markdown content for the lesson, including code examples and external resource links. Do NOT include exercises in the markdown — they are generated as structured data in the exercises field."
          ),
...
export const evaluationSchema = z.object({
  lessonComplete: z
    .boolean()
    .describe(
      "Whether the lesson is complete based on concept coverage and demonstrated understanding"
    ),
  masteryUpdates: z
    .array(
      z.object({
    ...
    .describe(
      "Concept mastery level updates. Only include concepts that were actually covered."
    ),
  insights: z
    .array(
      z.object({
        kind: z.enum(["strength", "gap", "preference", "pattern"]).describe(
          "strength: something the learner understands well. gap: a concept they struggle with. preference: how they learn best (analogies, code-first, etc). pattern: recurring behavior (asks clarifying questions, rushes through exercises, etc)."
        ),
        observation: z.string().describe("A concise observation about the learner"),
      })
    )
    .describe("Insights about the learner from this session — what they're good at, what they struggle with, how they learn best, behavioral patterns"),
  recommendation: z
    .enum(["advance", "repeat", "review"])
    .describe(
      "advance: lesson complete, move forward. repeat: significant gaps, redo. review: mostly understood, review weak points."
    ),
  planChanges: z
    .array(
      z.object({
        ...
        lessonTitle: z
          .string()
          .optional()
          .describe("Title for new or target lesson"),
        concepts: z
          .array(z.string())
          .optional()
          .describe("Concepts for the new lesson"),
        reason: z.string().describe("Why this change is needed"),
        afterLesson: z
          .number()
          .optional()
          .describe("Insert after this lesson number"),
      })
    )
    .optional()
